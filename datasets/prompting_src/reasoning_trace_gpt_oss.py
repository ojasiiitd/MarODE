...
def extract_reasoning(text):
    """
    Extract reasoning span marked by <Rstart> and <Rend> tags
    that appears after the last 'assistantfinal' marker.
    """
    # 1. Locate the LAST occurrence of 'assistantfinal'
    last_marker_idx = text.rfind("assistantfinal")
    if last_marker_idx == -1:
        return "[NONE]"  # 'assistantfinal' marker not found

    # 2. Slice text after the last 'assistantfinal'
    post_marker_text = text[last_marker_idx + len("assistantfinal"):]

    # 3. Search for reasoning tags only in this section
    match = re.search(r"<Rstart>(.*?)<Rend>", post_marker_text, re.DOTALL)
...
if __name__ == "__main__":
    ...
    # Prompting
    # ---------------------------
    output_dir = f"/home/ojas/scripts/datasets/RTs/RT_GPT_OSS_20B_{N_SHOT}Shot"
    ...
    with open("claim_ids_gptoss.txt", "r") as f:
    ...
    for prompt, claim_id in iter_prompts(n_shot=N_SHOT): # from make_prompt.py
        ...
        if len(results) == BATCH_SIZE:
            output_json_path = os.path.join(output_dir, f"trances__o{file_counter}.json")
...
# python -u reasoning_trace_gpt_oss.py --gpu_index 2 --n_shot 1 > LOGS_gpt_oss20b_1shot_reruns.txt
